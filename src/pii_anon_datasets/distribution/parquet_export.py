...
def export_parquet(
    ...
    *conformed* to that schema (a missing key becomes null, an unexpected key is ignored), so records
    ...
    writer: Any = None
    schema: Any = None  # canonical schema (stable column order), fixed from the first batch
    ...
    def _flush(rows: list[dict[str, object]]) -> None:
        ...
        The schema is fixed once from the first batch, but its columns are pinned in sorted order and
        EVERY row (in every batch) is conformed to it — a key absent from a record becomes null, an
        unexpected key is ignored. Corpus records carry the same fields in different native key orders
        (and some omit optional fields such as ``token_count``); without conforming, pyarrow would
        infer a divergent per-batch schema and ``write_table`` would raise ``Table schema does not
        match schema used to create file`` on the first divergent batch (the real corpus died mid-
        stream at row 110,000). The first batch is assumed representative — it carries every column
        the corpus uses (true for the 10k default ``batch_size`` over the 575,604-record corpus).
        """
        nonlocal writer, schema
        if not rows:
            return
        if schema is None:
            # Infer names + types from the first batch, then pin a deterministic (sorted) column
            # order so later batches whose records list the same fields in a different order conform.
