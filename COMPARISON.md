...
Head-to-head comparison of PII-Anon Evaluation Dataset v2.0.0 against major competing benchmarks for PII detection, anonymization, and re-identification resistance evaluation.

> **Sourcing note.** Competitor figures below are as published by their authors (as of 2026); verify against primary sources before citing. Only PII-Anon's own row reflects this repository's canonical metadata. **`PII-Bench` and "PIIBench" are two DISTINCT benchmarks** — the `PII-Bench` row in the tables below is the 2.8K query-aware semantic benchmark (2024); "PIIBench" as it appears in some other surveys refers to a different effort and is not the dataset tabulated here.
...
| Tier | What It Evaluates | PII-Anon v2.0 | Nemotron | AI4Privacy | Gretel | PII-Bench | TAB | PIILO | SPY | BigCode |
|------|-------------------|:-:|:-:|:-:|:-:|:-:|:-:|:-:|:-:|:-:|
...
| **Baselines** | **11-detector F2 leaderboard (8 local + 3 cloud DLP), LLM, CoNLL export** | GLiNER-PII | DistilBERT | GLiNER | LLM baselines | Custom eval | Fine-tuned GPT | Presidio | StarPII |
...
- **PII-Anon advantage**: ~440x scale, 60 languages, 4 domains, 7 evaluation dimensions, sensitivity classification inspired by TAB's approach
