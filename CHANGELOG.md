...
**Breaking schema release + first running scoring harness.** Produced via a full
developer-assistant PDLC pass (assessment → discovery → requirements → design →
development); spec lives under `dev-assist-artifacts/`.

### Changed (BREAKING)
- **`tier3_evaluation` wrapper**: the Tier-3 re-identification-resistance signal —
  `re_identification_resistance_score`, `estimated_reid_recall`, `tier3_risk_level`
  (moved out of `privacy_risk`) and top-level `behavioral_signals` — is consolidated
  into one `tier3_evaluation` object per record (the shape the README documented).
  The 7,003 paired-profile records' existing `tier3_evaluation` is merged, not clobbered.
- **Deterministic `record_id`**: now `uuid5` of `sha256(text + sorted annotation offsets)`,
  replacing the order-dependent counter. Prior id retained at `provenance.v1_3_0_record_id`.
- **Canonical entity-type registry** (`pii_anon_datasets.taxonomy`): **63 types / 9 categories**,
  derived from the actual corpus — single source of truth, fixing the prior 48/65/80
  documentation drift. `validate.py` now derives its sets from it.
- Per-record `version`/`schema_version` are `2.0.0`.

### Added
- **Scoring harness** (`pii_anon_datasets.scoring`): Hexagonal core with a documented
  span-matching I/O contract (deterministic, integer-count strict matching; partial
  overlap reported separately and excluded from CIs); detection scorer (P/R/F1/F2 with
  Wilson confidence intervals); `RRSResult` value object whose anti-anonymity caveat is
  a mandatory, non-strippable field.
- **Stats core** (`pii_anon_datasets.stats`): Wilson score intervals (integer-count guard).
- **Migration** `scripts/v1_3_0_to_v2_0_0.py` (+ `pii_anon_datasets.migration`); back-compat
  `pii_anon_datasets.compat.to_v1_record()`.
- **First test suite** (pytest; ~95% line+branch coverage on scorer/stats modules) + pytest/coverage
  config + declared optional-dependency groups (`dev`/`baselines`/`llm`/`distribution`).

### Migration
v1.3.0 is pinned at git tag `v1.3.0`. Run `PYTHONPATH=src python scripts/v1_3_0_to_v2_0_0.py`
(deterministic — byte-identical across runs). Legacy consumers: `compat.to_v1_record(rec)`.
`load_dataset()` signature is unchanged.

### Added — Statistical-power enrichment (S-PWR)
- **Corpus grown for statistical power**: 159,891 → **575,604** records (+415,713) /
  **2,486,438** annotations (+1,246,801) via the synthetic-lattice enrichment.
- **~72% of records** carry `provenance.source_type=synthetic_lattice_enrichment`.
- **All 710 committed lattice cells powered**; `PYTHONPATH=src python scripts/validate.py --lattice`
  NFR-018 power gate **PASS**.
- Pre-enrichment corpus pinned at git tags `v1.3.0` + `pre-lattice-enrichment`.
- **Epistemic note**: the enrichment is *formulaic* — it raises statistical power but is **not**
  evidence of real-world generalization (synthetic-distribution power ≠ external validity). The
  159,891 `tier3_evaluation` records remain the ~27.8% re-identification EVALUATION substrate.
