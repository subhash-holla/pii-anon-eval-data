#!/usr/bin/env python3
"""
Enrich the canonical dataset with query_context, adversarial metadata,
k-anonymity estimates, and regulatory domain tags.
...
INPUT_GZ = DATA_DIR / "pii_anon.jsonl.gz"
...
def enrich_regulatory_domains(records: list[dict]) -> int:
    ...
    for rec in records:
        ...
        # SOX (Sarbanes-Oxley) — financial records with salary/tax/bank info
        sox_types = {"BANK_ACCOUNT_NUMBER", "SALARY", "TAX_ID", "INVOICE_NUMBER"}
        if domain == "financial" and entity_types & sox_types:
            reg.add("sox")

        # LGPD (Brazil) — Portuguese language records
        if lang == "pt":
            reg.add("lgpd")

        # PIPA (South Korea) — Korean language records
        if lang == "ko":
            reg.add("pipa")

        # PCI-DSS — credit card data
        if entity_types & {"CREDIT_CARD_NUMBER", "CREDIT_CARD_FRAGMENT"}:
            reg.add("pci_dss")

        new_reg = sorted(reg)
        if new_reg != rec.get("regulatory_domains", []):
