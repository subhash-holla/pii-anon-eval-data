...
from pii_anon_datasets.migration import migrate_record, SCHEMA_VERSION
...
def main():
    ...
    for src_path in [GENERATED, COVERAGE_FILL, V120_GENERATED, V130_GENERATED, LATTICE_FILL, ART9_GENERATED, NON_PERSONAL]:
        if src_path.exists():
            ...
            with open(src_path, "r", encoding="utf-8") as f:
                for line in f:
                    rec = json.loads(line)
                    # Normalize any v1-shaped generator output to v2.0.0 (§0 schema-gap fix);
                    # idempotent for already-migrated records (e.g. the lattice fill).
    ...
    print(f"  Total generated: {len(generated)} records ({normalized} normalized to v2.0.0)")
    ...
    # 3c. Stable, content-deterministic order by record_id (content-addressed) so the .gz is
    # byte-identical across machines/re-runs (NFR-004). One-time reshuffle vs the historical order.
    merged.sort(key=lambda r: r["record_id"])
