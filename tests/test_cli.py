...
def test_fr_024_cli_export_dispatches_to_distribution(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def _fake_export_parquet(records, output, *, schema=None):  # noqa: ANN001, ANN202 - test double
        captured["records"] = records
        captured["output"] = output
        captured["schema"] = schema
        # Drain one record to prove the CLI hands over a real (streaming) iterator.
        captured["first"] = next(iter(records), None)
    ...
    # The CLI computed the FULL union schema (first pass) and handed it to the exporter so no column
    # that appears only in later records is dropped.
    assert captured["schema"] is not None and "record_id" in captured["schema"].names
